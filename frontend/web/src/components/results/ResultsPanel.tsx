...
function revealed(index: number, revealCount: number | undefined) {
  return revealCount === undefined || revealCount > index;
}

export default function ResultsPanel({ result, projectId, revealCount }: { result: AnalyzeResult; projectId?: string; revealCount?: number }) {
  ...
  return (
    <Box ref={containerRef} sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      <Box sx={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        {/* Feature Banner + Grounding alerts — section 0 */}
        {revealed(0, revealCount) && (
          <Box sx={FADE_UP}>
            <Box sx={{
              ...
              <Box sx={{ px: "10px", py: "4px", borderRadius: "999px", bgcolor: status.bg }}>
            ...
            {result.kb_sparse && (
              <Alert severity="warning" icon={<WarningAmberIcon sx={{ fontSize: 18 }} />} sx={{ borderRadius: 2, mt: 2 }}>
                <strong>Sparse knowledge base</strong> — Upload BRD/SRS for grounded test cases.
              </Alert>
            )}
            {result.apis_inferred && (
              <Alert severity="info" icon={<InfoOutlinedIcon sx={{ fontSize: 18 }} />} sx={{ borderRadius: 2, mt: 2 }}>
        ...
        {/* 01 Feature Understanding — section 1 */}
        {revealed(1, revealCount) && (
          <Box sx={FADE_UP}>
        ...
        {/* 02 Impacted Modules — section 2 */}
        {revealed(2, revealCount) && (
          <Box sx={FADE_UP}>
        ...
        {/* 03 Event Flow — section 3 */}
        {revealed(3, revealCount) && (result.event_flow?.length ?? 0) > 0 && (
          <Box sx={FADE_UP}>
        ...
        {/* 04 Risk Areas — section 4 */}
        {revealed(4, revealCount) && (
          <Box sx={FADE_UP}>
                  ...
                  ? result.risk_areas.map((r, i) => {
                      ...
                      return (
                        <Box key={i} sx={{
                          ...
                          <Box sx={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 3, bgcolor: rc.dot }} />
                          <Box sx={{ pl: 0.5, flex: 1, minWidth: 0 }}>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 1 }}>
                              ...
                              {r.module && <Typography sx={{ fontSize: 12, color: "text.secondary" }}>— {r.module}</Typography>}
                            </Box>
                          ...
                          {r.risk_score !== undefined && (
                            <Box sx={{ flexShrink: 0, textAlign: "right" }}>
                              ...
                              <Typography sx={{ fontSize: 20, fontWeight: 800, color: rc.text, lineHeight: 1 }}>{r.risk_score.toFixed(1)}</Typography>
                            </Box>
                          )}
                        </Box>
                      );
        ...
        {/* 05 Warnings — section 5 */}
        {revealed(5, revealCount) && (
          <Box sx={FADE_UP}>
                ...
                ? (
                  <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                    {result.heads_up_warnings.map((w, i) => (
                      <Box key={i} sx={{ display: "flex", gap: 2, p: 2, bgcolor: "#fffbeb", borderRadius: "10px", border: "1px solid #fde68a" }}>
                        ...
                        <Box>
                          ...
                          {w.recommendation && (
                            <Typography sx={{ fontSize: 12.5, color: "#78350f", mt: 0.5, lineHeight: 1.5 }}>→ {w.recommendation}</Typography>
                          )}
                        </Box>
                      </Box>
                    ))}
              ...
              <Box sx={{ display: "flex", gap: 1.5, alignItems: "center", mb: 2.5, flexWrap: "wrap" }}>
                <Box sx={{
                  ...
                  <input
                    ...
                    style={{ border: "none", outline: "none", background: "transparent", fontSize: 13, color: "inherit", flex: 1, fontFamily: "Inter, system-ui, sans-serif" }}
                  />
                </Box>
        ...
        {/* 07 Gherkin — section 7 */}
        {revealed(7, revealCount) && (
          <Box sx={FADE_UP}>
        ...
        {/* 08 Regression Suite — section 8 */}
        {revealed(8, revealCount) && (
          <Box sx={FADE_UP}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column" }}>
                {result.regression_suite?.map((r, i) => (
                  <Box key={i} sx={{
                    ...
                    <Box sx={{ px: "8px", py: "3px", borderRadius: "6px", flexShrink: 0, bgcolor: r.priority === "MUST-RUN" ? "#fef2f2" : "background.default" }}>
                    ...
                    {r.reason && <Typography sx={{ fontSize: 12, color: "text.disabled", maxWidth: 240, textAlign: "right" }}>{r.reason}</Typography>}
                  </Box>
                ))}
              </Box>
            </SectionCard>
        ...
        {/* 09 Coverage Gaps — section 9 */}
        {revealed(9, revealCount) && (
          <Box sx={FADE_UP}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                {result.missing_coverage?.length
                  ? result.missing_coverage.map((g, i) => (
                      <Box key={i} sx={{ p: 2.5, borderRadius: "10px", bgcolor: "background.default", border: "1px solid", borderColor: "divider" }}>
                        <Typography sx={{ fontSize: 13.5, fontWeight: 600, color: "text.primary", mb: g.recommendation ? 0.5 : 0 }}>
                          {g.area ?? g.description}
                        </Typography>
                        {g.recommendation && <Typography sx={{ fontSize: 12.5, color: "text.secondary" }}>→ {g.recommendation}</Typography>}
                      </Box>
                    ))
        ...
        {/* 10 API & Event Validation — section 10 */}
        {revealed(10, revealCount) && (
          <Box sx={FADE_UP}>
            <SectionCard id="sec-api" num={10} title="API & Event Validation" count={result.api_event_validation?.length ?? 0}>
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {result.api_event_validation?.map((api, i) => (
                  <Box key={i} sx={{ p: 2.5, borderRadius: "12px", bgcolor: "background.default", border: "1px solid", borderColor: "divider" }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 1.5 }}>
                      <Box sx={{ px: "8px", py: "3px", borderRadius: "6px", bgcolor: "#dbeafe" }}>
                        <Typography sx={{ fontSize: 11, fontWeight: 800, color: "#1d4ed8", fontFamily: "monospace" }}>{api.method}</Typography>
                      </Box>
                      <Typography sx={{ fontSize: 13, fontFamily: "monospace", color: "text.primary", fontWeight: 500 }}>{api.endpoint}</Typography>
                    </Box>
