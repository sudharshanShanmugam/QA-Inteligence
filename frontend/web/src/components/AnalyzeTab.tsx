...
export default function AnalyzeTab({ kbChunks, projectId, onAnalyzeComplete, onAnalysisDone, historyOnly = false, llmSettingsVersion = 0 }: { kbChunks: number; projectId: string; onAnalyzeComplete?: () => void; onAnalysisDone?: (context: string) => void; historyOnly?: boolean; llmSettingsVersion?: number }) {
  ...
  const runAnalysis = async (ans: Record<string, string>) => {
    ...
    try {
      ...
      setRevealCount(0);
      setPhase("done");
      // Progressively reveal each result section one by one
      if (revealTimerRef.current) clearInterval(revealTimerRef.current);
      const MAX_REVEAL = 14; // 1 summary + 11 content sections + 1 action buttons + 1 buffer
      let count = 0;
      revealTimerRef.current = setInterval(() => {
        count += 1;
        setRevealCount(count);
        if (count >= MAX_REVEAL) clearInterval(revealTimerRef.current!);
      }, 400);
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      {result && phase === "done" && (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {baselineResult && revealCount >= 1 && <ComparePanel baseline={baselineResult} current={result} />}
          {revealCount >= 1 && <ResultSummary result={result} />}
          <ResultsPanel result={result} projectId={projectId} revealCount={revealCount - 1} />
          {/* Action buttons — appear after all sections revealed */}
          {revealCount >= 13 && (
            <Box sx={{
              display: "flex", justifyContent: "center", gap: 2, pt: 1,
              animation: "fadeUp 0.4s ease both",
              "@keyframes fadeUp": { from: { opacity: 0, transform: "translateY(12px)" }, to: { opacity: 1, transform: "translateY(0)" } },
            }}>
              <Box
