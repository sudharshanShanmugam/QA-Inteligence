...
class LLMClient:
    ...
    def generate_risk_assessment(
        ...
        prompt = (
            ...
            "═══ OUTPUT FORMAT ═══\n\n"
            "Return a JSON array — one object per distinct risk area (max 10), sorted by risk_score descending.\n"
            ...
            '  "bug_cluster": "one-phrase pattern from the bug list (e.g. \'auth bypass\', \'null pointer\', \'data corruption\') or null"\n'
            '  "failure_propagation": "brief description of downstream impact if this area fails — which modules/users are affected"\n'
            '  "peak_time_risk": "high" | "medium" | "low" — how much worse this risk is during high-traffic windows\n\n'
        ...
        for item in result:
            ...
            item.setdefault("bug_cluster", None)
            item.setdefault("failure_propagation", "Impact scope unknown — upload dependency docs for analysis")
            item.setdefault("peak_time_risk", "medium")
            valid.append(item)
    ...
    def generate_security_scenarios(
        ...
        from test_generator.prompt_templates import SECURITY_SCENARIOS_PROMPT

        apis_text = json.dumps([
            {"endpoint": a.get("endpoint", a.get("name", "")), "method": a.get("method", "GET")}
            for a in apis[:8]
        ], default=str)
        roles_text = ", ".join(roles[:6]) if roles else "Not specified"

        prompt = SECURITY_SCENARIOS_PROMPT.format(
            feature_name=feature_name or "Unknown feature",
            user_story=user_story[:1200],
            apis=apis_text,
            roles=roles_text,
            rag_context=rag_context[:6000],
        )
        ...
        for idx, s in enumerate(result, 1):
            if "id" not in s:
                s["id"] = f"SEC-{idx:03d}"
            s.setdefault("type", "security")
            s.setdefault("scenario_type", "security")
        return result
