...
4. Every scenario must include a traceability comment on the first Then step:
   Then [assertion] # traces_to: [AC-X.X.X | US-NNN | story keyword]
5. Tags: use @regression plus one of @integration @smoke @critical @data-integrity @event-flow
6. Scenario titles must follow pattern: "Regression – [ImpactedModule]: [what interaction is being tested]"
7. Use concrete module names, not generic placeholders.
8. Do NOT generate unit-level tests for the changed feature itself — only integration regressions.
9. If impacted modules list is empty, generate {scenario_limit} regression scenarios for the module itself.
...
Feature: Regression suite for {feature_name}
  ...
  Scenario: Regression – [Module Name]: [interaction description]
    ...
    Then [the impacted module should still behave correctly] # traces_to: [AC-X.X | US-NNN]
...
- 429: rate limit exceeded — only if the endpoint is write or search
- 500: upstream dependency failure or DB error (note: verify graceful error response)
- IDOR: access another user's resource by substituting their ID in the URL path
- CSRF: submit a state-changing request without the CSRF token or from a foreign origin
- Data leakage: verify error messages do NOT expose stack traces, DB schema, or internal paths
- Cross-tenant: if multi-tenant, verify user A cannot read or modify user B's data even with valid auth
...
[
  {{
    ...
    "validations": [
      ...
      "429: POST /api/example returns 429 after exceeding rate limit threshold",
      "500: POST /api/example returns 500 with safe error code (no stack trace) when DB is unavailable",
      "IDOR: GET /api/example/{{other_user_id}} returns 403 when the caller does not own that resource",
      "CSRF: POST /api/example without CSRF token or from foreign origin returns 403",
      "Data leakage: 500 error response body contains only a safe error code, not a stack trace or SQL query",
      "Cross-tenant: authenticated user from tenant A cannot retrieve or modify records belonging to tenant B"
    ],
    "event_triggers": [
...
SECURITY_SCENARIOS_PROMPT = """You are a Senior Security QA Engineer generating security-focused test scenarios for a feature.
Use ONLY the data provided below — do not invent endpoints or roles not listed.
...
Generate security test scenarios covering the following attack classes.
Only include a class if it is applicable to the feature based on the data above.

1. IDOR (Insecure Direct Object Reference)
   – Access another user's resource by changing an ID in the URL or request body.
   – Expected: 403 Forbidden or 404 Not Found — never the other user's data.

2. CSRF (Cross-Site Request Forgery)
   – Submit a state-changing request (POST/PUT/DELETE) from a different origin without the anti-CSRF token.
   – Expected: 403 Forbidden.

3. Cross-tenant data isolation
   – If the system is multi-tenant, authenticate as tenant A and attempt to read or modify tenant B's records.
   – Expected: 403 or empty result — never tenant B's data.

4. Sensitive data in error messages
   – Trigger a server error (invalid FK, DB timeout simulation) and inspect the response body.
   – Expected: only a safe error code; no SQL, stack trace, or internal file paths.

5. Rate limiting / brute-force protection
   – Submit the same write operation or login attempt rapidly (e.g. 100 requests/minute).
   – Expected: 429 Too Many Requests after threshold; no data corruption.

6. Privilege escalation
   – Authenticated as a lower-privilege role, attempt an action that requires a higher role.
   – Expected: 403 Forbidden — request rejected, no partial execution.

7. Mass assignment / parameter pollution
   – Include extra fields in the request body (e.g. "role": "admin", "is_verified": true) that should be read-only.
   – Expected: extra fields silently ignored; user role or status unchanged.
...
[
  {{
    ...
    "attack_class": "IDOR",
    "title": "Short human-readable title of the security scenario",
    "preconditions": ["User A is authenticated", "User A has resource with id=123"],
    "steps": [
      "Authenticate as User A",
      "Send GET /api/resource/456 where 456 belongs to User B",
      "Inspect the response status and body"
    ],
    "expected_result": "403 Forbidden — User B's resource is not returned",
    "risk_level": "high"
  }}
]"""
