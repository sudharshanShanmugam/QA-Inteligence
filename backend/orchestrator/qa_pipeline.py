...
class QAPipeline:
    ...
    def _assess_complexity(
        ...
        if score <= 5:
            level = "simple"
            limits = {"functional": 10, "pairwise": 3, "dt": 3,
                      "state": 2, "flow": 3, "edge_count": 5, "gherkin": 5}
        elif score <= 10:
            level = "moderate"
            limits = {"functional": 16, "pairwise": 6, "dt": 6,
                      "state": 4, "flow": 4, "edge_count": 8, "gherkin": 8}
        else:
            level = "complex"
            limits = {"functional": 25, "pairwise": 10, "dt": 10,
                      "state": 7, "flow": 6, "edge_count": 12, "gherkin": 12}
    ...
    def run(self, user_story: str, project_id: str = "default", clarifying_answers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        failure_path_tests = event_flow_tracer.generate_failure_path_tests(feature_name, apis, events)
        ...
        coverage_metrics = coverage_analyzer.coverage_summary(
            features=all_features,
            test_cases=test_cases_in_graph,
            apis=apis,
            events=events,
        ...
        # Grounding score: 0.0 = pure LLM guess, 1.0 = fully grounded in KB + graph
        # Formula: 40% RAG similarity, 30% KB coverage (chunks), 30% graph context
        _rag_score   = min(1.0, top_score)
        _kb_score    = min(1.0, rag_result["total_retrieved"] / 15.0)
        _graph_score = min(1.0, (len(modules) + len(graph_bugs) + len(graph_data.get("events", []))) / 10.0)
        grounding_score = round(0.40 * _rag_score + 0.30 * _kb_score + 0.30 * _graph_score, 3)
        ...
        # Collect user roles from graph context for security scenario generation
        _roles_for_security = list({
            m.get("role", m.get("name", ""))
            for m in (modules + impacted_modules)
            if m.get("role") or "role" in m.get("name", "").lower()
        })

        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as _executor:
            ...
            _f_security = _executor.submit(
                llm_client.generate_security_scenarios,
                feature_name=feature_name,
                user_story=user_story,
                apis=apis[:8],
                roles=_roles_for_security,
                rag_context=rag_context_str,
            )
            ...
            security_scenarios = _f_security.result()

        # ──────────────────────────────────────────────────────────────────
        # Assemble Final Output
        # ──────────────────────────────────────────────────────────────────
        overall_risk = feature_risk["priority"]
        total_scenarios = len(analytical_scenarios) + len(gherkin_tests) + len(edge_cases) + len(security_scenarios)

        result = {
            "feature_understanding": feature_understanding,
            "impacted_modules": self._format_modules(modules, impacted_modules),
            "event_flow": flow_steps,
            "failure_path_tests": failure_path_tests,
            ...
            # grounding_score: 0.0 = pure LLM inference, 1.0 = fully KB + graph grounded
    ...
    def _enrich_regression_entries(
        ...
        def _make_trace(entry: Dict[str, Any], priority: str) -> Dict[str, Any]:
            ...
            # Build traces_to: collect any explicit requirement/story refs
            # already on the entry, plus synthesise from the trigger tag.
            existing_refs: List[str] = entry.get("traces_to", [])
            if isinstance(existing_refs, str):
                existing_refs = [existing_refs]
            ac_ref = entry.get("acceptance_criteria_ref", "")
            story_ref = entry.get("user_story_ref", "")
            trigger_ref = f"trigger:{trigger_tag}"
            traces_to = list(dict.fromkeys(
                [r for r in existing_refs if r]
                + ([ac_ref] if ac_ref else [])
                + ([story_ref] if story_ref else [])
                + [trigger_ref]
            ))
