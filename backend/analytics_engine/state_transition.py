...
class StateTransitionEngine:
    ...
    def generate_tests(self, machine: StateMachine) -> Dict[str, List[Dict[str, Any]]]:
        ...
        analysis = self.analyze_reachability(machine)
        return {
            ...
            "unreachable_states": analysis["unreachable_state_tests"],
            "dead_end_states": analysis["dead_end_state_tests"],
            "reachability_analysis": analysis["summary"],
        }

    def analyze_reachability(self, machine: StateMachine) -> Dict[str, Any]:
        """
        Detect unreachable states (never entered from initial) and dead-end states
        (no outgoing transitions and not a declared final state).
        Returns test cases and a summary dict.
        """
        adj_forward: Dict[str, Set[str]] = defaultdict(set)
        adj_backward: Dict[str, Set[str]] = defaultdict(set)
        for t in machine.transitions:
            adj_forward[t.from_state].add(t.to_state)
            adj_backward[t.to_state].add(t.from_state)
        ...
        while queue:
            ...
            if state in reachable:
                continue
            reachable.add(state)
            queue.extend(adj_forward.get(state, []))

        all_states = set(machine.states)
        declared_finals = set(machine.final_states)

        unreachable = all_states - reachable
        dead_ends = {
            s for s in all_states
            if not adj_forward.get(s) and s not in declared_finals
        }

        unreachable_tests = []
        for i, state in enumerate(sorted(unreachable), 1):
            unreachable_tests.append({
                "id": f"ST-UNREACH-{i:03d}",
                "type": "state_analysis",
                "scenario_type": "architecture_warning",
                "severity": "high",
                "title": f"ARCHITECTURAL ISSUE: State '{state}' is unreachable from initial state '{machine.initial_state}'",
                "description": (
                    f"No transition path leads from '{machine.initial_state}' to '{state}'. "
                    "This state can never be entered in normal operation — it is either "
                    "dead code or a missing transition definition."
                ),
                "recommendation": (
                    f"Add a transition that leads to '{state}', or remove it from the state machine definition. "
                    "An unreachable state indicates either a missing business rule or a specification gap."
                ),
                "preconditions": [f"System is in initial state: {machine.initial_state}"],
                "steps": [
                    f"Attempt to reach state '{state}' via any available event sequence",
                    "Verify whether the state is actually reachable in the running system",
                    "Compare with the specification to determine if a transition is missing",
                ],
                "expected_result": f"CLARIFICATION NEEDED: '{state}' should either be reachable or removed",
                "risk_level": "high",
            })

        dead_end_tests = []
        for i, state in enumerate(sorted(dead_ends), 1):
            dead_end_tests.append({
                "id": f"ST-DEADEND-{i:03d}",
                "type": "state_analysis",
                "scenario_type": "architecture_warning",
                "severity": "medium",
                "title": f"POTENTIAL ISSUE: State '{state}' is a dead end (no exit transitions)",
                "description": (
                    f"State '{state}' has no outgoing transitions and is not declared as a final state. "
                    "Once entered, the entity is permanently trapped with no way to progress."
                ),
                "recommendation": (
                    f"Either add exit transitions from '{state}', declare it as a final state in the spec, "
                    "or verify that 'stuck in this state' is the intended business behaviour."
                ),
                "preconditions": [f"{machine.entity} has entered state: {state}"],
                "steps": [
                    f"Move {machine.entity} to state '{state}'",
                    "Attempt every known event/action from this state",
                    "Verify whether the system allows any further state change",
                ],
                "expected_result": (
                    f"Either: a) '{state}' is confirmed as terminal and documented as such, "
                    f"or b) missing transitions are identified and added"
                ),
                "risk_level": "medium",
            })

        return {
            "unreachable_state_tests": unreachable_tests,
            "dead_end_state_tests": dead_end_tests,
            "summary": {
                "total_states": len(all_states),
                "reachable_states": sorted(reachable),
                "unreachable_states": sorted(unreachable),
                "dead_end_states": sorted(dead_ends),
                "declared_final_states": sorted(declared_finals),
                "has_issues": bool(unreachable or dead_ends),
            },
        }
